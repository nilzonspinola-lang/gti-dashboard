// GTI Dashboard v5 — login direto em app.controlle.com
const { chromium } = require('playwright');
const fs = require('fs');

(async () => {
  console.log('▶ GTI Dashboard v5');
  ...
  // LOGIN direto em app.controlle.com
  console.log('→ Abrindo app.controlle.com/login...');
  await page.goto('https://app.controlle.com/login', { waitUntil: 'domcontentloaded', timeout: 30000 });
  await page.waitForTimeout(3000);
  console.log('→ URL login:', page.url());
  console.log('→ Campos:', await page.evaluate(() => {
    return [...document.querySelectorAll('input')].map(i => i.type + '|' + i.name + '|' + i.placeholder).join(', ');
  }));
  ...
  console.log('→ Credenciais enviadas, aguardando redirect...');

  try { await page.waitForURL(u => !u.includes('login'), { timeout: 20000 }); } catch(e) { console.log('timeout:', page.url()); }
  await page.waitForTimeout(5000);

  const urlDash = page.url();
  console.log('→ URL pós-login:', urlDash.replace(/accessToken=[^&\s]+/g, '***'));
